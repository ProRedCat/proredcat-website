...
Here’s the important twist, OpenRouter does return the final cost in USD (via `include_usage: true`). So although we can’t compute the cost straight away, we can work backwards from the cost and solve for the one missing variable.
...
$$
W= \frac{  \text{Cost}_{\text{USD}} \cdot 10^{6}  - T_{p} \, P_{\text{base}}  - C \left( P_{\text{read}} - P_{\text{base}} \right)  - T_{c} \, P_{\text{out}}}{  P_{\text{write}} - P_{\text{base}}}
$$
