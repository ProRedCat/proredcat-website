...
date: 2024-08-16
description: "Discover how I built an Alexa Skill to track public transport, tackling performance challenges and optimizing within the constraints of free hosting. Dive into the technical journey behind creating a smart, efficient voice assistant."
published: true
---
...
<div className="flex justify-center space-x-4" >
    <Image src="/blog/alexa-skill-announcement-wellington-transport/bus_stop_saved.jpg" width={240} height={0}/>
    <Image src="/blog/alexa-skill-announcement-wellington-transport/next_bus.jpg"  width={240} height={0}/>
</div>

<div className="flex justify-center space-x-4 mt-4" >
    <Image src="/blog/alexa-skill-announcement-wellington-transport/train_station_saved.jpg" width={240} height={0}/>
    <Image src="/blog/alexa-skill-announcement-wellington-transport/next_train.jpg"  width={240} height={0}/>
</div>
...
Our final approach addressed both the developer maintainability and user experience by using the `Result<Ok<T>, Err>` pattern so we can fail up (combined with a really nice architecture, but that could be a whole other blog). We also made use of logging through [Cloudwatch](https://aws.amazon.com/cloudwatch/) and crash (error) reporting through [Raygun](https://raygun.com/) to provide post deployment monitoring.
...
Re-prompting is one of the most important usability features we explored and implemented. We could have sent a reply saying "this is not valid", but to enhance user experience it's nicer to be asked again so they do not have to repeat the whole phrase.
...
Circling back to when I mentioned sourcemaps earlier, we can finally go a little bit deeper. When we build our project we get our `index.js` file and a `index.js.map` file. This `.map` file is the [secret ingredient](https://youtu.be/b2F-DItXtZs?t=141) in the sourcemap sauce. It stores the mappings from line/column numbers to their original source, the source code can also be embedded in the sourcemap. This means that when you symbolicate the sourcemap you can also get snippets of code from where that line is mapped to.
...
If you enjoyed this blog I do have a bunch more planned so stay tuned. They'll likely be technical deepdives on topics I've found interesting as of late, such as using [ASP.NET MVC](https://dotnet.microsoft.com/en-us/apps/aspnet/mvc) with [HTMX](https://htmx.org/), or some upcoming projects for WearOS apps.